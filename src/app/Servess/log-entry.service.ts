...
export class LogEntryService {
  ...
  // Get all log entries with optional filters and pagination
  getAllLogEntries(searchDto: LogEntrySearchDto): Observable<LogEntryDto[]> {
    ...
    return this.http.get<LogEntryDto[]>(this.logEntryApiUrl, { headers, params });
  }

  // Get a log entry by ID
  getLogEntryById(id: number): Observable<LogEntryDto> {
    const headers = this.getHeaders();
    return this.http.get<LogEntryDto>(`${this.logEntryApiUrl}/${id}`, { headers });
  }

  // Create a new log entry
  createLogEntry(logEntry: LogEntryCreateDto): Observable<LogEntryDto> {
    const headers = this.getHeaders();
    return this.http.post<LogEntryDto>(this.logEntryApiUrl, logEntry, { headers });
  }

  // Update a log entry
  updateLogEntry(id: number, logEntry: LogEntryUpdateDto): Observable<void> {
    const headers = this.getHeaders();
    return this.http.put<void>(`${this.logEntryApiUrl}/${id}`, logEntry, { headers });
  }
